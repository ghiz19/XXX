...
public class EquipementResource {
    ...
    public ResponseEntity<List<EquipementDTO>> getAllEquipements(Pageable pageable, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get a page of Equipements");
        Page<EquipementDTO> page;
        if (eagerload) {
            page = equipementService.findAllWithEagerRelationships(pageable);
        } else {
            page = equipementService.findAll(pageable);
        }
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, String.format("/api/equipements?eagerload=%b", eagerload));
